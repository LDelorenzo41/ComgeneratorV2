...
import {
  ...
  PHASE_HEADING_PATTERN,
  extractTextFromChildren,
} from '../../lib/phaseExtractor';
...
interface LessonMarkdownRendererProps {
  ...
  className?: string;
  /**
   * Si fourni, un bouton "Générer un support" est affiché à côté des
   * titres de phase (Échauffement, Phase 1, etc.), comme dans l'aperçu.
   */
  onOpenExerciseModal?: (phaseHeading: string) => void;
}

/**
 * Rendu Markdown d'une séance, identique à EnhancedMarkdownRenderer mais
 * avec en plus le bouton "Générer un support" sur les titres de phase.
 * Composant dédié (additif) : EnhancedMarkdownRenderer reste inchangé pour
 * ses autres usages (ex. ExerciseGeneratorModal).
 */
const LessonMarkdownRenderer: React.FC<LessonMarkdownRendererProps> = ({
  content,
  className,
  onOpenExerciseModal,
}) => {
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ children }) => {
            ...
            return (
              <h3 className="text-base font-medium text-gray-900 dark:text-gray-100 mb-2 mt-4 flex items-center gap-2 flex-wrap">
                <span>{children}</span>
          ...
          pre: ({ children }: any) => {
            ...
            if (React.isValidElement(child)) {
              ...
              if (cls.includes('language-mermaid') || cls.includes('language-chart')) {
                return <>{children}</>;
              }
            }
