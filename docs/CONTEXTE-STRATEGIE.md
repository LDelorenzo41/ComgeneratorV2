...
| `deleted_users_blacklist` : e-mails de comptes supprimés conservés sans durée, et crus sans finalité | **partiellement réglé, avec incident (06/09/2026)** — la table a été vidée et son alimentation retirée de `delete_user_account()`, mais sa **suppression a cassé l'inscription** en production (« Database error saving new user »). `public.handle_new_user()`, appelée par le trigger `on_auth_user_created`, la consulte : la finalité existait bien, elle était implémentée côté base et non côté application — la recherche du 18/08 ne couvrait que `src/` et les Edge Functions. Table rétablie vide par `20260906c`. Le garde-fou anti-réinscription est de ce fait **inerte** : arbitrage à trancher, voir ci-dessous |
| **Règle tirée de l'incident du 06/09** : aucun `DROP` d'objet sur ce projet sans inventaire préalable des dépendances **côté base** — `SELECT proname FROM pg_proc WHERE prosrc ILIKE '%<objet>%'` — la recherche dans le dépôt ne voit pas les objets créés depuis le dashboard | **à appliquer systématiquement** |
| `delete_user_account()` supprimait 8 tables sur 14 : scénarios, banque de réponses, documents importés, chunks, conversations et dossiers RAG survivaient à la suppression du compte, ainsi que les fichiers du bucket `rag-documents` | **✅ réglé (06/09/2026)** — fonction réécrite depuis sa définition de production, suppression explicite table par table sans dépendre des cascades, plus le Storage ; `SET search_path` ajouté. Migration `20260906b_fix_delete_user_account.sql` |
| Tentative de correction (`20260906b`) : la suppression de compte est devenue impossible | **annulée (06/09/2026)** — `storage.objects` porte un trigger `protect_objects_delete` BEFORE DELETE **FOR EACH STATEMENT** qui refuse toute suppression SQL directe (ERRCODE 42501, « Use the Storage API instead »). Un trigger STATEMENT se déclenche même sur un DELETE sans ligne concernée : l'échec était systématique. Retour arrière par `20260906d`. Les fichiers doivent passer par l'API Storage, pas par SQL |
| **Règle élargie après l'incident du 06/09** : avant d'écrire pour la première fois dans une table d'un schéma non versionné, recenser ses TRIGGERS autant que ses dépendances — `SELECT tgname, pg_get_triggerdef(oid) FROM pg_trigger WHERE tgrelid = '<table>'::regclass AND NOT tgisinternal` | **à appliquer systématiquement** |
| Aucun médiateur de la consommation désigné, alors que la vente de tokens à des enseignants relève du B2C (un salarié n'exerce ni activité commerciale ni libérale : c'est un consommateur) | **assumé (06/09/2026)** — adhésion ~15 €/an pour un chiffre d'affaires inférieur. Contrepartie inscrite aux CGV : engagement contractuel de remboursement sous 14 jours (insatisfaction bornée à 30 jours, pour rester dans la fenêtre de remboursement Stripe ; défauts sans limite de temps), plus favorable que le droit légal, qui vise à régler tout litige avant qu'il n'appelle une médiation. Risque résiduel connu : sanction jusqu'à 3 000 €. S'éteint avec l'arrêt des encaissements, prévu avant mai 2027 |
